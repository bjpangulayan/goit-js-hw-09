...
function promiseGenerator(event) {
    ...
    for(let positionValue = 1; positionValue <= amount.value; positionValue++){
        createPromise(positionValue, delayValue).then(({ position , delay: delay1  })=>{
            (0, _notiflixNotifyAio.Notify).success(`Fulfilled promise ${position} in ${delay1}ms`);
        }).catch(({ position , delay: delay2  })=>{
            (0, _notiflixNotifyAio.Notify).failure(`Rejected promise ${position} in ${delay2}ms`);
        });
